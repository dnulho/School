...
{
	os << static_cast<int>(obj.m_month) << "/" << static_cast<int>(obj.m_day)
...
{
	if (m_valid)
		cout << static_cast<int>(m_day) << " " << m_monthNames[m_month - 1]
...
{
	if (m_valid)
		cout << static_cast<int>(m_month) << "/" << static_cast<int>(m_day)
		<< "/" << static_cast<int>(m_year);
	else
		cout << "Invalid Date";
}
...
{
	m_day++;
	if (m_day > daysArray[m_month-1] || m_day == 0)
	{
		m_day = 1;
		m_month++;
		if (m_month > 12)
...
const int cDate::daysArray[12] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
