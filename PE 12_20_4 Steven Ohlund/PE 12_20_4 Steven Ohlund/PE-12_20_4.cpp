...
bool RemoveIng(char **& ingList, int & numIng, int index)
{
	...
	{
		cout << "The passed array is empty, can't remove anything." << endl;
	}
	else
	{
		cout << "Error: RemoveIng() was passed invalid list values" << endl;
	}
	return good;
}
...
void EditIng(char **& ingList, int & numIng, int index, char * newIng)
{
	...
	else
	{
		...
		while (index < numIng - 1 &&_stricmp(newIng, ingList[index + 1]) > 0)
		{
			ingList[index] = ingList[index + 1];
			index++;
		}
		while ( index > 0 && _stricmp(newIng, ingList[index - 1]) < 0)
		{
			ingList[index] = ingList[index - 1];
			index--;
		}
...
void RemoveIngredient(char **& ingList, int & numIng)
{
	...
	cin.clear();
	cin.ignore(cin.rdbuf()->in_avail());
	int index = FindIng(ingList, numIng, buffer);
	if (RemoveIng(ingList, numIng, index))
	{
		cout << buffer << " removed\n";
	}
}
